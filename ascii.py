...
def ascii_write(atoms,filename):
    if filename=="screen":
        ...
        f= open(filename,"w")
        eref = -60048.574632555
        epot = float(atoms.epot)
        epot = 27.211385e+00*epot
        f.write("E_FOR_STRUCTURE_IN_EQB., E_AIMS_FOR_THIS_STRUCTURE\n")
        f.write("%24.12f%24.12f\n" % (eref,epot))
        #f.write("%d\n" % atoms.nat)
       ...
       # f.write("%24.15E%24.15E%24.15E\n" % (atoms.cellvec[2][0],atoms.cellvec[2][1],atoms.cellvec[2][2]))
        for i in range(atoms.nat):
            ...
            y=atoms.rat[i][1]
            z=atoms.rat[i][2]
            f.write("%6s%24.15E%24.15E%24.15E%5s\n" % ('atom\t',x,y,z,atoms.sat[i]))
